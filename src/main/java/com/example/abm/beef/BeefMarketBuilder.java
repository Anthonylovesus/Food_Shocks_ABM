...
import com.example.abm.beef.agents.*;
...
public class BeefMarketBuilder extends DefaultContext<Object> implements ContextBuilder<Object> {

  // varieties and purchase priority
  private static final List<String> VARIETIES =
      Arrays.asList("BULLS", "HEIFERS", "STEERS");
  ...
  static {
    WHOLESALE_MARKUP.put("STEERS", 0.15);
    WHOLESALE_MARKUP.put("HEIFERS", 0.10);
    WHOLESALE_MARKUP.put("BULLS", 0.08);
  }
  ...
  static {
    TASTE_MEAN.put("STEERS", 1.6);  TASTE_STD.put("STEERS", 0.10);
    TASTE_MEAN.put("HEIFERS", 1.4); TASTE_STD.put("HEIFERS", 0.05);
    TASTE_MEAN.put("BULLS", 0.0);TASTE_STD.put("BULLS", 0.0);
  }
  ...
  public Context build(Context<Object> context) {
    ...
    // agents
    producers.add(new ProducerAgent(1, "BULLS", P, ml));
    producers.add(new ProducerAgent(2, "HEIFERS",          P, ml));
    producers.add(new ProducerAgent(3, "STEERS",    P, ml));
    ...
    schedule.schedule(
    		  ...
    		  (IAction) () -> BeefMarketBuilder.this.writeOutputs()
    		);
  ...
  public void writeOutputs() {
	  ...
	  System.out.println("[BeefMarketBuilder] Writing CSVs to: " + dir.getAbsolutePath());
	  
	  try {
	    tracker.writeCSVs(outDir);
	  } catch (Exception ex) {
	    System.err.println("[BeefMarketBuilder] tracker.writeCSVs failed:");
	    ex.printStackTrace();
	  }

	  System.out.println("[BeefMarketBuilder] writeOutputs completed.");
	}
