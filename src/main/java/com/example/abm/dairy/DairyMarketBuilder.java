package com.example.abm.dairy;

import com.example.abm.dairy.agents.*;
import com.example.abm.dairy.MathUtil;
...
public class DairyMarketBuilder extends DefaultContext<Object> implements ContextBuilder<Object> {
  ...
  static {
    WHOLESALE_MARKUP.put("2 PERCENT", 0.10);
  }
  ...
  static {
    TASTE_MEAN.put("2 PERCENT", 0.10);        TASTE_STD.put("2 PERCENT", 0.05);
  }
  ...
  public Context build(Context<Object> context) {
    ...
    // agents
    producers.add(new ProducerAgent(1, "2 PERCENT", P, ml));
    ...
    schedule.schedule(
    		  ...
    		  (IAction) () -> DairyMarketBuilder.this.writeOutputs()
    		);
  ...
  public void writeOutputs() {
	  ...
	  System.out.println("[DairyMarketBuilder] Writing CSVs to: " + dir.getAbsolutePath());
	  
	  try {
	    tracker.writeCSVs(outDir);
	  } catch (Exception ex) {
	    System.err.println("[DairyMarketBuilder] tracker.writeCSVs failed:");
	    ex.printStackTrace();
	  }

	  System.out.println("[DairyMarketBuilder] writeOutputs completed.");
	}
