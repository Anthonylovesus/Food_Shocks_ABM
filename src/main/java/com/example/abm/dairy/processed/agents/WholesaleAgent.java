package com.example.abm.dairy.processed.agents;

import com.example.abm.dairy.processed.ModelParams;
import com.example.abm.dairy.processed.agents.ProducerAgent;
import com.example.abm.dairy.processed.MathUtil;
...
public class WholesaleAgent {
  ...
  public void procure(List<ProducerAgent> producers,
	...
	double yld = MathUtil.clamp(P.getINPUT_TO_OUTPUT_YIELD(), 1e-6, 1.0);
	...
	for (String v : varieties) {
		...
		int outputNeed = Math.max(0, target + backlog - have);    // end product units needed
		if (outputNeed <= 0) continue;

		// ---- input/output conversion ----
		int inputNeed = (int) Math.ceil(outputNeed / yld);        // input product needed
		...
		int remainingInput = inputNeed;

		for (ProducerAgent p : producers) {
			if (!v.equals(p.getVariety())) continue;
			if (remainingInput <= 0) break;
			
	        int bought = p.sellToWholesaler(this, remainingInput, /*packMultiple*/10);
	        if (bought > 0) {
	          addStock(v, bought);
	          remainingInput -= bought;
			}
		}
	}
  }
  ...
  private double estimateBulkCost(List<ProducerAgent> producers, String variety) {
	  ...
	  double bulkOutput = sum / n;
	  double yld = MathUtil.clamp(P.getINPUT_TO_OUTPUT_YIELD(), 1e-6, 1.0);
	  return bulkOutput / yld; // effective bulk cost per output product unit
	}
  ...
  //private double estimateBulkCost(List<ProducerAgent> producers, String variety) {
    ...
    //}
    //if (n == 0) {
      //double ask = askByVar.getOrDefault(variety, 0.7);
      //return ask / 1.10; 
    //}
    //return sum / n;
  //}
