...
public class DataTracker {
  ...
  public void writeCSVs(String outDir) throws IOException {
    ...
    writeSimpleTable(dir.resolve("dairy_timeseries.csv"),
    ...
    for (Map.Entry<Integer, List<Double>> e : producerPrice.entrySet()) {
      ...
      int n = Math.max(price.size(), Math.max(sold.size(), rev.size()));
      Path f = dir.resolve("dairy_producer_" + pid + ".csv");
      try (BufferedWriter bw = Files.newBufferedWriter(f)) {
        bw.write("iteration,price,units_sold_cum,revenue_cum\n");
        for (int i = 0; i < n; i++) {
          bw.write((i+1) + "," + val(price, i) + "," + ival(sold, i) + "," + val(rev, i));
          bw.write("\n");
        }
      }
    }
